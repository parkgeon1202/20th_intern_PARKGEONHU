...
void subscribe_node::initialize(){
    this->declare_parameter<double>("link_bottom", 0.0);
    this->declare_parameter<double>("link_middle", 0.0);
    this->declare_parameter<double>("link_upper", 0.0);    
    this->declare_parameter<double>("angle_bottom", 0.0);
    this->declare_parameter<double>("angle_middle", 0.0);
    this->declare_parameter<double>("angle_upper", 0.0);

        // 파라미터 읽기
    double L1 = this->get_parameter("link_bottom").as_double();
    double L2 = this->get_parameter("link_middle").as_double();
    double L3 = this->get_parameter("link_upper").as_double();
    double a0 = this->get_parameter("angle_bottom").as_double();
    double a1 = this->get_parameter("angle_middle").as_double();
    double a2 = this->get_parameter("angle_upper").as_double();

    emit init_func(L1, L2, L3, a0, a1, a2);
}